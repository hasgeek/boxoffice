...
def get_settlements(filename):
    ...
    for trans in transactions:
        if trans[11]:
            ...
            if trans[1] == 'payment':
                payment = OnlinePayment.query.filter_by(pg_paymentid=trans[0], pg_payment_status=RAZORPAY_PAYMENT_STATUS.CAPTURED).first()
                if payment:
                    pt = PaymentTransaction.query.filter_by(online_payment=payment, transaction_type=TRANSACTION_TYPE.PAYMENT).one_or_none()
                    # Get settlement
                ...
                if payment:
                    refund_transactions = PaymentTransaction.query.filter_by(online_payment=payment, transaction_type=TRANSACTION_TYPE.REFUND).all()
                    # settlement_amount = [tr for tr in transactions if tr[0] == trans[11]][0][4]
                    settlement = [tr for tr in transactions if tr[0] == trans[11]]
                    if settlement:
...
def get_line_items(filename):
    ...
    transactions = csv_to_rows(filename)
    line_items = []
    header = ['line_item_id', 'order_id', 'settlement_id', 'line_item_title', 'cancelled']
    for trans in transactions:
        if trans[1] == 'payment':
            payment = OnlinePayment.query.filter_by(pg_paymentid=trans[0]).first()
            for line_item in payment.order.line_items:
                line_items.append([line_item.id, line_item.order.id, trans[11], line_item.item.title, line_item.status == LINE_ITEM_STATUS.CANCELLED])
...
def get_orders(settlement_filename):
    ...
    for settlement_dict in settlement_dicts:
        if settlement_dict['type'] == 'payment' and settlement_dict['entity_id'] not in [pord['entity_id'] for pord in payment_orders]:
            payment = OnlinePayment.query.filter_by(pg_paymentid=settlement_dict['entity_id'], pg_payment_status=RAZORPAY_PAYMENT_STATUS.CAPTURED).one()
            payment_transaction = PaymentTransaction.query.filter_by(online_payment=payment, transaction_type=TRANSACTION_TYPE.PAYMENT).one()
            payment_orders.append({
                ...
                'receivable_amount': payment_transaction.amount - Decimal(settlement_dict['fee']),
            ...
            try:
                ...
                payment_transaction = PaymentTransaction.query.filter_by(amount=Decimal(settlement_dict['debit']), online_payment=payment, transaction_type=TRANSACTION_TYPE.REFUND).first()
                if payment_transaction:
                    payment_orders.append({
                        ...
                        'fee': settlement_dict['fee'],
                        'receivable_amount': Decimal(0) - Decimal(settlement_dict['debit']),
                        'settlement_date': settlement_dict['settled_at'],
    ...
    with open('settlement_orders.csv', 'w') as outputcsv:
        fieldnames = ['entity_id', 'settlement_id', 'order_id', 'buyer_fullname', 'credit', 'debit', 'fee', 'receivable_amount', 'transaction_amount', 'settlement_date', 'transaction_date']
