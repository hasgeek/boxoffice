...
$(function() {
  ...
  boxoffice.initResources = function(config) {
    this.config.resources = {
      itemCollection: {
        method: 'GET',
        urlFor: function(codes){
          if (codes.length > 0) {
            var codeParams = "?";
            for (var codeIndex = 0; codeIndex < codes.length; codeIndex++) {
              codeParams += "code=" + codes[codeIndex];
              if (codeIndex < codes.length - 1) {
                codeParams += "&";
              }
            }
            return boxoffice.config.baseURL + '/ic/' + config.ic + codeParams;
          }
  ...
  boxoffice.init = function(widgetConfig) {
    ...
    $.ajax({
      url: boxoffice.config.resources.itemCollection.urlFor(boxoffice.util.getDiscountCodes()),
      crossDomain: true,
