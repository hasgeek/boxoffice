...
def validate_unique_discount_code_base(form, field):
    if DiscountPolicy.query.filter(DiscountPolicy.id != form.edit_id, DiscountPolicy.discount_code_base == field.data).notempty():
        raise StopValidation(_('This discount coupon prefix already exists. Please pick a different prefix'))
...
class DiscountPriceForm(forms.Form):
    ...
    start_at = forms.DateTimeField(_("Price start date"),
        validators=[forms.validators.DataRequired(_("Please specify a start date and time"))])
    end_at = forms.DateTimeField(_("Price end date"),
...
class DiscountCouponForm(forms.Form):
    ...
    coupon_code = forms.StringField(_("Code for discount coupon"),
