...
class Price(BaseScopedNameMixin, db.Model):
    ...
    __tableargs__ = (db.UniqueConstraint('item_id', 'name'),
                     db.CheckConstraint('valid_from < valid_to',
                                        'validity_bound'))
    ...
    def __repr__(self):
        return u'<Price "{price}" for item "{item}">'\
                .format(price=self.title, item=self.item.title)

    @classmethod
    def current(cls, item):
        now = datetime.utcnow()
        return cls.query.filter(cls.item == item,
                                cls.valid_from <= now,
                                cls.valid_upto >= now)\
