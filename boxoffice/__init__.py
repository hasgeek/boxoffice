...
# Load assets into config from the manifest generated by webpack
with app.open_resource(app.config['ASSETS_MANIFEST_PATH']) as f:
    asset_bundles = json.loads(f.read())
    for asset_key, asset_path in asset_bundles['assets'].items():
        app.config[asset_key] = asset_path
