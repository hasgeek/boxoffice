...
def tickets_report(menu: Menu) -> Response:
    ...
    def row_handler(row: list[Any]) -> list[Any]:
        # localize datetime
        row_list = [
            (
                v
...
def attendees_report(menu: Menu) -> Response:
    ...
    for ticket in menu.tickets:
        if ticket.assignee_details:
            for detail in ticket.assignee_details:
    ...
    def row_handler(row: list[dict | datetime | str]) -> dict[str, Any]:
...
def orders_api(organization: Organization, menu: Menu) -> Response:
...
def invoices_report(organization: Organization) -> Response:
    today = date.today()
    ...
    def row_handler(row: list) -> dict[str, Any]:
        order = Order.query.get(row[order_id_index])
        assert order is not None  # noqa: S101  # nosec B101
        row = list(row)
        row.insert(buyer_name_index, order.buyer_fullname)
        row.insert(buyer_email_index, order.buyer_email)
        dict_row = dict(list(zip(headers, row, strict=False)))
...
def invoices_report_zb(organization: Organization) -> Response:
    today = date.today()
    period_type = request.args.get('type', 'all')
    invoice_filter: dict[str, Any] = {}
    if period_type == 'monthly':
    ...
    def row_handler(row: list[Any]) -> dict[str, Any]:
        order = Order.query.filter(Order.id == row[order_id_index]).one()
...
def settled_transactions(organization: Organization) -> Response:  # noqa: ARG001
