...
window.Modernizr = (function (i, e, u) {
  ...
  d.flexbox = function () {
    ...
    (function (k, g, r, x) {
      g += ':';
      k.style.cssText =
        (g + q.join(r + ';' + g)).slice(0, -g.length) + (x || '');
  ...
  d.touch = function () {
    return (
      'ontouchstart' in i ||
      Q('@media (' + q.join('touch-enabled),(') + 'modernizr)')
    );
  };
  ...
  d.svg = function () {
    return (
      !!e.createElementNS && !!e.createElementNS(v.svg, 'svg').createSVGRect
    );
  };
  ...
  d.svgclippaths = function () {
    return (
      !!e.createElementNS &&
      /SVG/.test(O.call(e.createElementNS(v.svg, 'clipPath')))
    );
  };
  for (var H in d)
    if (R(d, H)) {
  ...
  i.attachEvent &&
    ...
    (function (a, b) {
      ...
      a.attachEvent('onbeforeprint', function () {
        for (var p = -1; ++p < r; )
          for (
            var m = b.getElementsByTagName(g[p]), I = m.length, t = -1;
            ++t < I;

          )
            if (m[t].className.indexOf('iepp_') < 0)
              m[t].className += ' iepp_' + g[p];
