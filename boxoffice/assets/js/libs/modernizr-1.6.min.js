...
window.Modernizr = (function (i, e, u) {
  ...
  d.hashchange = function () {
    return (
      o('hashchange', i) && (document.documentMode === u || document.documentMode > 7)
    );
  };
  d.history = function () {
  ...
  d.smil = function () {
    return (
      !!e.createElementNS && /SVG/.test(O.call(e.createElementNS(v.svg, 'animate')))
    );
  };
  d.svgclippaths = function () {
    return (
      !!e.createElementNS && /SVG/.test(O.call(e.createElementNS(v.svg, 'clipPath')))
    );
  };
  for (var H in d)
    if (R(d, H)) {
