  ...
  casper.waitFor(
    function check() {
      return this.evaluate(function () {
        return (
          document.querySelectorAll('#conference #conference-ticket .increment')
            .length > 0
        );
      });
    },
      ...
      for (item in items[itemCategory]) {
        ...
        for (var i = 0; i < add; i++) {
          casper.evaluate(
            function (itemCategory, item) {
              document
                .querySelector('#' + itemCategory + ' #' + item + ' .increment')
                .click();
            },
            itemCategory,
            item
        ...
        for (var i = 0; i < subtract; i++) {
          casper.evaluate(
            function (itemCategory, item) {
              document
                .querySelector('#' + itemCategory + ' #' + item + ' .decrement')
                .click();
            },
            itemCategory,
            item
    ...
    for (var itemCategory in items) {
      for (item in items[itemCategory]) {
        ...
        if (discount) {
          test.assertEquals(
            casper.fetchText(
              '#' + itemCategory + ' #' + item + ' .discount-amount span'
            ),
            ' ' + discount,
            '#' + itemCategory + ' #' + item + ' discount exists and is correct'
          );
        } else {
          test.assertEquals(
            casper.fetchText(
            ...
            '',
            '#' + itemCategory + ' #' + item + " discount doesn't exist"
          );
        }
        ...
        if (subtotal) {
          test.assertEquals(
            casper.fetchText(
              '#' + itemCategory + ' #' + item + ' .subtotal span'
            ),
            ' ' + subtotal,
            '#' + itemCategory + ' #' + item + ' subtotal exists and is correct'
          );
        } else {
          test.assertEquals(
            casper.fetchText(
            ...
            '',
            '#' + itemCategory + ' #' + item + " subtotal doesn't exist"
          );
        }
      }
    }
