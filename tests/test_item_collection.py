...
class TestItemCollectionAPI(unittest.TestCase):
    ...
    expected_restricted_ticket_data = {
        "conference": {
            "conference-ticket": {
                "title": "Conference ticket",
                "price": 3500,
            ...
            "restricted-ticket": {
                ...
                "price": 3500,
                "description": '<p><i class="fa fa-calendar"></i>14 - 15 April 2016</p><p><i class="fa fa-map-marker ticket-venue"></i>MLR Convention Center, JP Nagar</p><p>This ticket is restricted.</p>',
    ...
    def setUp(self):
        ...
        self.ic = ItemCollection.query.first()
        self.resp = self.client.get('/ic/{ic}'.format(ic=self.ic.id), headers=[('X-Requested-With', 'XMLHttpRequest'), ('Origin', app.config['BASE_URL'])])
    ...
    def test_restricted_item_listing(self):
        resp = self.client.get('/ic/{ic}'.format(ic=self.ic.id), headers=[('X-Requested-With', 'XMLHttpRequest'), ('Origin', app.config['BASE_URL'])])
        data = json.loads(resp.data)
        for category in data['categories']:
            expected_items = self.expected_restricted_ticket_data[category['name']]
            self.assertEquals(sorted([c['name'] for c in category['items']]), sorted(expected_items.keys()))
