...
from boxoffice.models import DiscountCoupon, DiscountPolicy, Item, db
...
class TestKharchaAPI(unittest.TestCase):
    ...
    def test_expired_item_kharcha(self):
        ...
        self.assertIsNone(
            resp_json.get('line_items')[str(expired_ticket.id)].get('base_amount')
        )
    ...
    def test_discounted_bulk_kharcha(self):
        ...
        expected_discount_policy_ids = [
            str(
                DiscountPolicy.query.filter_by(title='10% discount on rootconf')
                .first()
                .id
            )
        ]
        ...
        for expected_policy_id in expected_discount_policy_ids:
            self.assertIn(expected_policy_id, policy_ids)
    ...
    def test_unlimited_coupon_kharcha(self):
        ...
        # Test that all the discount policies are returned
        for expected_policy_id in expected_discount_policy_ids:
            self.assertIn(str(expected_policy_id), policy_ids)
    ...
    def test_discount_policy_without_price_kharcha(self):
        ...
        self.assertEqual(
            resp_json.get('line_items')[str(first_item.id)].get('discounted_amount'),
            decimal.Decimal(0),
        )
    ...
    def test_zero_discounted_price_kharcha(self):
        ...
        # Test that all the discount policies are returned
        for expected_policy_id in expected_discount_policy_ids:
            self.assertNotIn(expected_policy_id, policy_ids)
